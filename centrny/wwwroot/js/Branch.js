/* Branch Management JS
   - Ensures jQuery is loaded before running (fixes "$ is not defined")
   - Uses Bootstrap 5 Modal API safely (no reliance on jQuery .modal plugin)
   - Replaces .modal('show'/'hide') with safe helpers
*/
...
(function boot(factory) {
    if (typeof window.jQuery !== 'undefined') {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            if (typeof window.jQuery !== 'undefined') {
                ...
                console.error('Branch.js requires jQuery. Please include jQuery before this script.');
            }
        });
    }
})(function ($) {
    ...
    async function loadCentersAndBranches(rootCode) {
        ...
        $('#add-center-btn').off().on('click', function () {
            ...
            $('#CenterAddress').val('');
            $('#CenterPhone').val('');
            safeShowModalById('addCenterModal');
        });
        ...
        $('.add-branch-btn').off().on('click', function () {
            ...
            $('#BranchPhone').val('');
            $('#BranchStartTime').val('');
            safeShowModalById('addBranchModal');
        });
    ...
    async function submitAddCenter() {
        ...
        try {
            ...
            if (res.ok) {
                safeHideModalById('addCenterModal');
                setTimeout(async () => {
                    await loadCentersAndBranches(currentRootCode);
    ...
    async function submitAddBranch() {
        ...
        try {
            ...
            if (res.ok) {
                safeHideModalById('addBranchModal');
                setTimeout(async () => {
                    await loadCentersAndBranches(currentRootCode);
