...
---------------------------------------------------*/
	...
	// so the visual dark text styling is effectively removed without editing every HTML file.
	document.addEventListener("DOMContentLoaded", function () {
		try {
			document
				.querySelectorAll(".text-dark-1, .text-dark")
				.forEach(function (el) {
					el.classList.remove("text-dark-1");
					el.classList.remove("text-dark");
				});
		} catch (e) {
			// fail silently
			console && console.error && console.error("remove-text-dark.js error", e);
		}
	});

	// Ensure the 'text-dark' class is removed from all elements on load and for any future DOM changes
	(function enforceNoTextDark() {
		function stripFromTree(root) {
			if (!root) return;
			if (root.classList && root.classList.contains("text-dark")) {
				root.classList.remove("text-dark");
			}
			const nodes = root.querySelectorAll
				? root.querySelectorAll(".text-dark")
				: [];
		...
		if (document.readyState === "loading") {
			document.addEventListener("DOMContentLoaded", function () {
				stripFromTree(document.body);
			});
		} else {
			stripFromTree(document.body);
		}

		// Observe future changes and strip 'text-dark' reactively
		try {
	...
	// Highlight the current page link in the header/menu
	function markActiveNavLink() {
		try {
			...
			var pathFile = fullPath
				.split("/")
				.pop()
				.split("?")[0]
				.split("#")[0]
				.toLowerCase();
			if (!pathFile) pathFile = "home.html"; // default to home

			var candidates = document.querySelectorAll(".js-navList a, .menu__nav a");
			candidates.forEach(function (a) {
				var rawHref = a.getAttribute("href") || "";
				if (!rawHref) return;
				var hrefFile = rawHref.split("/").pop().split("?")[0].split("#")[0];
				try {
